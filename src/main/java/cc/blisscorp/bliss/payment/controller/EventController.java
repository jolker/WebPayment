...
//        String game = (String) req.getSession().getAttribute(Constants.SESSION_GAME);
        ...
        if (user != null) {
            ...
            // current set default is nplay classic
            data.put(Constants.SESSION_GAME, "nplay_classic");
        }
        try {
            new FreeMarker(Constants.PAGE_EVENT, data).render(req, resp);
        } catch (Exception ex) {
            GA.app.error(EventController.class.getName() + " - " + ex.getMessage(), ex);
            SlackUtils.sendMessage("EventController failed. " + ex.toString());
        }
